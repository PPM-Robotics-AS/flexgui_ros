...
function editorService(
    ...
    var editHandler = {
        ...
        openProperties: function () {
            if (editHandler.selectedFidgets.length == 0) return;
        ...
        getFidgetSize: function (fidget) {
            ...
            return {
                width: parseInt(fidget.properties.width),
                height: parseInt(fidget.properties.height)
            };
        },
            ...
            function getFidgets(container, top, left) {
                angular.forEach(container.fidgets, function (fidget) {
                    ...
                    fidget.absoluteOffset = {
                        top: parseInt(parseInt(fidget.properties.top) + parseInt(top) + parseInt(containerOffset.top)),
                        left: parseInt(parseInt(fidget.properties.left) + parseInt(left) + parseInt(containerOffset.left))
                    };

                    fidgets.push(fidget);

                    if (fidget.source == "fidgetGroup") {
                        getFidgets(fidget, parseInt(fidget.absoluteOffset.top), parseInt(fidget.absoluteOffset.left));
                    }
                });
            }
        ...
        addScreenAccesibilities: [],
        editModeAccesibilities: [],

        isAddScreenAvailable: function () {
            var ret = true;

            angular.forEach(this.addScreenAccesibilities, function (item) {
                if (!item()) ret = false;
            });
        ...
        isEditAvailable: function () {
            ...
            angular.forEach(this.editModeAccesibilities, function (item) {
                if (!item()) ret = false;
            });
